...
export default function TimeGrid({ timeSlots = [], bookedTimes = new Set(), selectedTime, selectedTimes = [], onSelect, multiSelect = false }) {
  return (
    <div className="rounded-xl border border-[#374151] bg-[#111827] p-3">
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
        {timeSlots.map((t) => {
          const isBooked = bookedTimes.has(t);
          const isSelected = multiSelect ? selectedTimes.includes(t) : selectedTime === t;
          ...
          const selectedCls = isSelected ? ' ring-2 ring-[#f0c41b] bg-[#f0c41b]/20' : '';
          return (
            <button
              key={t}
              disabled={isBooked}
      ...
      <div className="flex items-center gap-3 mt-3 text-xs text-[#9ca3af]">
        ...
        {multiSelect && <div className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded bg-[#f0c41b]/60 ring-2 ring-[#f0c41b]" /> Selecionado</div>}
      </div>
    </div>
  );
}